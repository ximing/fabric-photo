...
description: 把现有项目重构为适合 Claude Code 或 CatPaw IDE 的持久化指令渐进式加载架构。用户提到“拆分根 CLAUDE.md”、“按目录下沉规则”、“给当前项目设计分层指令文件”、“创建不同目录的 CLAUDE.md”、“补 `.claude/rules` / `.catpaw/rules`”、“把仓库改造成渐进式加载”时使用。技能默认不仅解释加载机制，还会基于仓库结构直接设计并落地最合适的指令文件布局。
---
...
| 目标                                          | Claude Code 位置               | CatPaw IDE 位置                                                       |
| --------------------------------------------- | ------------------------------ | --------------------------------------------------------------------- |
| 所有人始终都要看到                            | 根 `CLAUDE.md`                 | `.catpaw/rules/*.md`（`ruleType: Always`）                            |
| 只有某个子树需要看到                          | 该子树下的 `CLAUDE.md`         | 子树下的 `.catpaw/rules/*.md`（`ruleType: Always`）                   |
| 横跨多个目录但可由 glob 命中                  | `.claude/rules/*.md` + `paths` | `.catpaw/rules/*.md`（`ruleType: Auto Attached` + `globs` + `paths`） |
| 当前用户个人、仅当前仓库适用                  | `CLAUDE.local.md`              | —                                                                     |
| 目录级规则已有 `.catpaw/rules/readme.md` 体系 | 维护 sibling `CLAUDE.md`       | 维护 `readme.md`                                                      |
| 一次性工作流、不是持久化规则                  | skill，不放进 `CLAUDE.md`      | skill，不放进 `.catpaw/rules/`                                        |
