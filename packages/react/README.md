...
- 布局：顶栏 48px、左工具栏 48px、右面板 240px（`fp-side-panel`：图层面板在上，max-height 45%，属性面板在下填满）、画布区灰底 `#e5e5e5`，grid 命名区域 `top/tools/canvas/props`（两行）；工具选项条为浮动层（`.fp-floating-options`，绝对定位于画布区顶部居中），不占 grid 行。
